...
Pour qu'un dashboard **lise et écrive des données persistantes** (tracking, assignations, notes, état), utiliser le schéma `dashboard_storage` de la base PostgreSQL applicative via l'endpoint `/api/query`. Activer le flag `has_persistence` en DB via `create_dashboard`/`update_dashboard`.
