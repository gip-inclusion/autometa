...
from lib.data_inclusion import _parse_pkey, execute_sql
...
def test_execute_sql_opens_tunnel_and_queries(mocker):
    ...
    mock_tunnel = mocker.MagicMock(local_bind_port=54321)
    ...
    mock_sa_result = mocker.MagicMock()
    mock_sa_result.keys.return_value = ["id", "source"]
    mock_sa_result.fetchall.return_value = [("dora--abc", "dora")]
    mock_sa_conn = mocker.MagicMock()
    mock_sa_conn.__enter__ = mocker.MagicMock(return_value=mock_sa_conn)
    mock_sa_conn.__exit__ = mocker.MagicMock(return_value=False)
    mock_sa_conn.execute.return_value = mock_sa_result
    mock_engine = mocker.MagicMock()
    mock_engine.connect.return_value = mock_sa_conn
    mocker.patch("lib.data_inclusion.create_engine", return_value=mock_engine)
