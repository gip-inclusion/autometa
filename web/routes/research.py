...
Vector math uses simsimd instead of numpy for efficiency: simsimd is a single
94 KB C extension (1 module) vs numpy's 84 modules and ~24 MiB. On our corpus
(~1700 chunks × 1024 dimensions), simsimd.cosine completes in <1 ms with
identical ranking results. This avoids loading numpy into the web server process
at all — numpy remains installed for data analysis scripts but is not imported
by the FastAPI app.
"""
...
def _blob_to_vec(blob):
    """Decode a sqlite BLOB (float32 little-endian) into an array.array."""
    a = array.array("f")
    a.frombytes(blob)
    return a
...
def _get_embeddings(conn):
    """Load and cache embeddings as array.array vectors.

    No normalization is needed: simsimd.cosine handles unnormalized vectors.
    """
    db_path = config.NOTION_RESEARCH_DB
    ...
    for row in rows:
        ...
        embeddings.append(_blob_to_vec(row["embedding"]))
...
def _embed_query(text):
    ...
    data = resp.json()
    return array.array("f", data["data"][0]["embedding"])
...
def _cosine_scores(query_vec, embeddings):
    ...
    simsimd.cosine returns distance (1 - similarity), so we invert.
    """
    return [1.0 - simsimd.cosine(vec, query_vec) for vec in embeddings]


def _ranked_indices(scores):
    """Return indices sorted by descending score (replaces np.argsort[::-1])."""
...
def search_corpus(q, limit=20, db_filter=None, type_filter=None):
    ...
    try:
        ...
        top_indices = _ranked_indices(scores)
