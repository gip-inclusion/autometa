...
def notify_alert_channel(message: str) -> None:
    """Post to SLACK_ALERT_CHANNEL, prefixed with the emitting app; silent if Slack is not configured."""
    ...
    try:
        if not post_message(token, channel, f"[{config.APP_NAME}] {message}"):
