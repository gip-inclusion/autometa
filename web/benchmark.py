"""Benchmark route: replay a conversation and measure agent performance."""
...
def _original_metrics(conv):
    """Extract timing and tool data from the stored conversation."""
    msgs = conv.messages
    ...
    for k, idx in enumerate(user_indices):
        ...
        t_end = msgs[user_indices[k + 1]].created_at if k + 1 < len(user_indices) else msgs[-1].created_at
...
def _history_for_prompt(msgs, prompt_idx: int) -> tuple[str, list[dict]]:
    """Build (prompt, history) for replaying the Nth user prompt.

    History contains only user/assistant text messages prior to the prompt,
    matching what CLIBackend._build_prompt expects.
    """
    user_indices = [i for i, m in enumerate(msgs) if m.type == "user"]
...
async def _run_prompt(backend, prompt: str, history: list[dict], tag: str) -> PromptResult:
    """Replay one user prompt through the agent backend and collect metrics."""
    result = PromptResult()
    t0 = time.monotonic()
    try:
