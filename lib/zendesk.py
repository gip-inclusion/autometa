...
def ticket_query(query: str) -> str:
    return query if "type:ticket" in query else f"{query} type:ticket".strip()
...
class ZendeskAPI:
    ...
    def search_tickets(
        ...
        """
        params: dict[str, Any] | None = {"query": ticket_query(query), "per_page": 100}
        if sort_by:
    ...
    def count_tickets(self, query: str) -> int:
        ...
        data = self._get("search/count.json", {"query": ticket_query(query)})
