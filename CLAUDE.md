...
| 2026-09-18 | fix | CI verify_dashboard : diff-cover (70 % → 99 %) par tests unit de `watch`/`audit` (Page et sync_playwright factices) ; parcours navigateur réparé en lançant l'audit dans un thread | Le job coverage ne fusionne que unit + integration : du code exercé seulement par `browser/` compte non couvert. `pytest browser` fait tourner `test_socle.py` d'abord, dont la boucle asyncio pytest-playwright (portée session) fait refuser tout `sync_playwright()` ultérieur dans le même thread — un test navigateur lancé seul passe et masque le bug : toujours rejouer `pytest browser` complet. |
